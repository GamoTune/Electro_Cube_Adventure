...
def clic_gauche (event): #Utilisation du clic gauche (placer un bloc, détruire, selectionner...)
    global listeNiveau, entryIDcle, boutonIDCleValidation, selectBlocID, couleurSet, textIDKey, padsouris, selectBlocCoordX, selectBlocCoordY, slectBloc, infoBouton, textIDTP1, textIDTP2, entryID1, entryID2, boutonIDTPValidation
            ...
            if xSourisCase == listeNiveau[padsouris]["coordsBloc"][0] and ySourisCase == listeNiveau[padsouris]["coordsBloc"][1]:
                if delonespebloc == True: #La gomme
                    ...
                    for blocinListeMonde in listeMonde:
                        if blocinListeMonde["coordsBloc"] == listeNiveau[padsouris]["coordsBloc"]:
        ...
        for blocinListeNiveau in listeNiveau:
            if xSourisCase == blocinListeNiveau["coordsBloc"][0] and ySourisCase == blocinListeNiveau["coordsBloc"][1]:
                ...
                if verifyEditKeyPorte == True:
                    ...
                    textIDTP1 = Label(fenetre_edit_bloc, text="ID 1", font=("Arial", int(24*ratioFenetre)))
                    textIDTP1.place(x=100*ratioFenetre,y=130*ratioFenetre,anchor=CENTER)

                    textIDTP2 = Label(fenetre_edit_bloc, text="ID 2", font=("Arial", int(24*ratioFenetre)))
                    ...
                    entryID1.place(x=100*ratioFenetre, y=175*ratioFenetre,anchor=CENTER)

                    entryID2 = Entry(fenetre_edit_bloc, bg="lightgrey", width=int(10*ratioFenetre), font=("Arial", int(24*ratioFenetre)))
                    entryID2.place(x=400*ratioFenetre, y=175*ratioFenetre,anchor=CENTER)
...
def getKeyID (IDValue): #Enregistre les infos d'une clé
    ...
    for blocinListeMonde in listeMonde:
        if selectBlocCoordX == blocinListeMonde["coordsBloc"][0] and selectBlocCoordY == blocinListeMonde["coordsBloc"][1] and blocinListeMonde["idLevel"] == str(id_level): #Permet de savoir si le bloc exite déjà dans la liste
                ...
                break
    if verify == False:
        listeMonde.append({"type" : slectBloc["typeBloc"], "idKeyPorte" : IDValue, "coordsBloc" : [selectBlocCoordX, selectBlocCoordY], "idLevel" : str(id_level), "keyCollect" : 0})
    save_world()
    print(listeMonde)
