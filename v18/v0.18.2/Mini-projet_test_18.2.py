...
def first_launch (): #La fonction "first_launch" permet de déclaré la plus part des variable global
    ...
    lienmonde = "assets/data/editeur/monde"+str(id_monde)
...
######################################################################## L'édition
    ...
    bouton_edit = Button(fenetre_bouton, image=imageWIP, bg='white', command=edit_objet)
...
def monde_finder (numMonde):
    ...
    id_monde = numMonde
    lienmonde = "assets/data/editeur/monde"+str(id_monde)+"monde_info.txt"
...
def menu_post_solo ():
    ...
    buttonStart = Button(menuPostSolo, image=imageWIP)
...
def edit_objet (event=0):
    global edit, fenetreeditTest, fenetre_edit_bloc
    if fenetreeditTest == False:
        ...
        infosLabel = Label(fenetre_edit_bloc, image=imageEditeurInfos)
        infosLabel.pack()
        ...
        while padsouris != len(dicoNiveau["listBloc"]):
            if xSourisCase == dicoNiveau["coordsBloc"][padsouris*2] and ySourisCase == dicoNiveau["coordsBloc"][padsouris*2+1]:
                if dicoNiveau["typeBloc"][padsouris] == 2:
                    pass"""


        #if bool(fenetre_edit_bloc.winfo_exists()) == False:
