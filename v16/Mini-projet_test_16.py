...
######################################################################## L'édition
    ...
    bouton_bloc_solide = Button(fenetre_bouton, image=imageBoutonEditeurBlocSolide, bg='white', command=lambda:typeBlocs("spawn"))
    bouton_bloc_solide.place(x=206,y=103)
...
def move_up (event):
    ...
    if edit == False:
        ...
        while pad != len(dicoNiveau["listBloc"]):
            if  positionJoueur[0] == dicoNiveau["coordsBloc"][pad*2] and positionJoueur[1]-1 == dicoNiveau["coordsBloc"][pad*2+1]:
                verify = True
            pad += 1
...
def move_left (event):
    ...
    if edit == False:
        ...
        while pad != len(dicoNiveau["listBloc"]):
            if  positionJoueur[0]-1 == dicoNiveau["coordsBloc"][pad*2] and positionJoueur[1] == dicoNiveau["coordsBloc"][pad*2+1]:
                verify = True
            pad += 1
...
def loadTestZone (event=0):
    ...
    while rang < len(dicoNiveau["typeBloc"]):
        if dicoNiveau["typeBloc"][rang] == 0:
            couleurBloc = 'black'
        elif dicoNiveau["typeBloc"][rang] == 1:
            couleurBloc = 'blue'
...
"""
Note :
problème quand on charge un niveau (l'encient part pas)
