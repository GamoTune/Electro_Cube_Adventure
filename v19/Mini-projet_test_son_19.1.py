...
def plassage_de_block (event):
    global dicoNiveau, entryIDcle, boutonIDCleValidation, selectBloc, couleurSet, testIDKey, padsouris
...
def getKeyID (IDValue):
    ...
    while pad != len(dicoMonde["coords_bloc"])/2:
        if dicoMonde["coords_bloc"][pad*2] in dicoNiveau["coordsBloc"] and dicoMonde["coords_bloc"][pad*2+1] in dicoNiveau["coordsBloc"]:
            verify = True
        pad += 1
    print(pad)
    print(dicoMonde)
