...
######################################################################## L'édition
    ...
    bouton_bloc_solide = Button(fenetre_bouton, image=imageBoutonEditeurBlocSolide, bg='white', command=lambda:choix_bloc("spawn"))
...
def move_up (event):
    ...
    if edit == False:
        ...
        while pad != len(dicoNiveau["listBlocSolide"]):
            if  positionJoueur[0] == dicoNiveau["coordsBlocSolide"][pad*2] and positionJoueur[1]-1 == dicoNiveau["coordsBlocSolide"][pad*2+1]:
                verify = True
            pad += 1
...
def move_left (event):
    ...
    if edit == False:
        ...
        while pad != len(dicoNiveau["listBlocSolide"]):
            if  positionJoueur[0]-1 == dicoNiveau["coordsBlocSolide"][pad*2] and positionJoueur[1] == dicoNiveau["coordsBlocSolide"][pad*2+1]:
                verify = True
            pad += 1
...
################################################################### Fonction en édition ###################################################################
def choix_bloc (blocpossable):
    global blocSolide
    global blocSpawn
    blocSolide = False
    blocSpawn = False
    if blocpossable == "solide":
        blocSolide = True
    elif blocpossable == "spawn":
        blocSpawn = True
    else:
        pass

def plassage_de_blocs (event):
    global dicoNiveau
    ...
    if blocSolide == True:
        padsouris = 0
        verify = False
        while padsouris != len(dicoNiveau["listBlocSolide"]):
            if xSourisCase == dicoNiveau["coordsBlocSolide"][padsouris*2] and ySourisCase == dicoNiveau["coordsBlocSolide"][padsouris*2+1]:
                verify = True
            padsouris += 1

        if edit == True and verify == False:
            dicoNiveau["listBlocSolide"].append(canevas.create_rectangle(xSourisCase*nombrePixel,ySourisCase*nombrePixel,xSourisCase*nombrePixel+nombrePixel,ySourisCase*nombrePixel+nombrePixel, fill='black'))
            dicoNiveau["coordsBlocSolide"].append(xSourisCase)
            dicoNiveau["coordsBlocSolide"].append(ySourisCase)

    if blocSpawn == True:
        padsouris = 0
        verify = False
        if dicoNiveau["BlocSpawn"] != False:
            verify = True
        padsouris += 1

        if edit == True and verify == False:
            dicoNiveau["listBlocSpawn"].append(canevas.create_rectangle(xSourisCase*nombrePixel,ySourisCase*nombrePixel,xSourisCase*nombrePixel+nombrePixel,ySourisCase*nombrePixel+nombrePixel, fill='blue'))
            dicoNiveau["coordsBlocSpawn"].append(xSourisCase)
            dicoNiveau["coordsBlocSpawn"].append(ySourisCase)
    print(dicoNiveau["listBlocSpawn"])

def deleteblock (event=0):
    global dicoNiveau
    
    padblocklist = 0
    while padblocklist < len(dicoNiveau["listBlocSolide"]) and edit == True:
        canevas.delete(dicoNiveau["listBlocSolide"][padblocklist])
        padblocklist += 1
    padblocklist = 0
    while padblocklist < len(dicoNiveau["listBlocSpawn"]) and edit == True:
        canevas.delete(dicoNiveau["listBlocSpawn"][padblocklist])
        padblocklist += 1
    dicoNiveau["listBlocSolide"].clear()
    dicoNiveau["coordsBlocSolide"].clear()
    dicoNiveau["listBlocSpawn"].clear()
    dicoNiveau["coordsBlocSpawn"].clear()

def deleteLastBlock (event=0):
    global dicoNiveau

    if len(dicoNiveau["listBlocSolide"]) > 0 or len(dicoNiveau["listBlocSpawn"]) > 0 and edit == True:
        if dicoNiveau["listBlocSolide"][-1] > dicoNiveau["listBlocSpawn"][-1]:
            canevas.delete(dicoNiveau["listBlocSolide"][-1])
            del dicoNiveau["listBlocSolide"][-1]
            del dicoNiveau["coordsBlocSolide"][-1]
        else:
            canevas.delete(dicoNiveau["listBlocspawn"][-1])
            del dicoNiveau["listBlocSpawn"][-1]
            del dicoNiveau["coordsBlocSpawn"][-1]
...
def loadTestZone (event=0):
    ...
    padblocklist = 0
    while padblocklist < len(dicoNiveau["listBlocSolide"]):
        canevas.delete(dicoNiveau["listBlocSolide"][padblocklist])
        padblocklist += 1
    dicoNiveau["listBlocSolide"].clear()
    dicoNiveau["coordsBlocSolide"].clear()
    ...
    padblocklist = 0
    while padblocklist < len(dicoNiveau["listBlocSpawn"]):
        canevas.delete(dicoNiveau["listBlocSpawn"][padblocklist])
        padblocklist += 1
    dicoNiveau["listBlocSpawn"].clear()
    dicoNiveau["coordsBlocSpawn"].clear()
    ...
    if os.path.exists(lienfichier):
        ...
        rang = 0
        while rang < len(dicoNiveau["listBlocSolide"]):
            canevas.create_rectangle(dicoNiveau["coordsBlocSolide"][(rang*2)]*nombrePixel, dicoNiveau["coordsBlocSolide"][(rang*2+1)]*nombrePixel, dicoNiveau["coordsBlocSolide"][(rang*2)]*nombrePixel+nombrePixel, dicoNiveau["coordsBlocSolide"][(rang*2+1)]*nombrePixel+nombrePixel, fill='black')
            rang += 1
        while rang < len(dicoNiveau["listBlocSpawn"]):
            canevas.create_rectangle(dicoNiveau["coordsBlocSpawn"][(rang*2)]*nombrePixel, dicoNiveau["coordsBlocSpawn"][(rang*2+1)]*nombrePixel, dicoNiveau["coordsBlocSpawn"][(rang*2)]*nombrePixel+nombrePixel, dicoNiveau["coordsBlocSpawn"][(rang*2+1)]*nombrePixel+nombrePixel, fill='blue')
            rang += 1
...
fenetre.bind("<B1-Motion>", lambda event : plassage_de_blocs(event))
fenetre.bind("<Button-1>", lambda event : plassage_de_blocs(event))
...
"""
Note :
La couleur change tout les blocs quand on recharge le niveau :/
Trouver une soluce pour supp "listeBlocs"
