...
def first_launch (): #La fonction "first_launch" permet de déclaré la plus part des variable global
    ...
    coordsBlocs = [] #Liste des coordonnées virtuel de tout les blocs d'un niveau organiser en paire (ex : coords X Y du 1er bloc = [0,0] donc si on rajoute les coords du 2e bloc = [0,0,1,1]) [default : []]
    ...
    dicoSave = {"coordsBlocs" : [], "tpJoueur" : []} #Dictionnaire des informations d'un niveau (comme les coordonnées des blocs) qui sont enregistrer
