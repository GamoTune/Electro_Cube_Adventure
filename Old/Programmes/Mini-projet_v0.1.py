...
def menu_edition():
    ...
    boutonEditPlayTest = Button(menuEditionFrame, image=imageBoutonPlayTest, relief='groove', bd=0, bg='black') #command=play_test
    ...
    if etatJeu == "solo":
        ...
        for bloc in monde[niveau]:
            if bloc["x"] == posJTestX and bloc["y"] == posJTestY:
                if bloc["type"] == 0: pass
                elif bloc["type"] == 2 and bloc["collect"] == 1: ramasse_cle(indexBloc)
                elif bloc["type"] == 3 and bloc["collect"] == 1: ouvre_porte(indexBloc)
                elif bloc["type"] == 4: tp_joueur(indexBloc)
                ok = False
            indexBloc += 1
        if ok == True:
            print("GO")
            if positionJoueur[xyJ] == posMax: #Si la position du joueur est egale à la limite de l'écran (ou de la map)
                id_level[xyJ] = id_level[xyJ]+posJF
                print("changement niveau")
                cacher_niveau()
        ...
        for bloc in monde[niveauTest]:
            if bloc["type"] == 4:
                if bloc["idAssoc"][0] == monde[niveau][indexBloc]["idAssoc"][1]:
                    id_level = [niveauTest[0],niveauTest[1]]
                    cacher_niveau()
                    get_niveau(id_level) #La fonction level_search est appeler
                    affichage_niveau()
...
def change_color (c): #Change la couleur du bloc selectionner
    global newColorBloc
    match couleurSet:
        case 1: newColorBloc = 'red'
        case 2: newColorBloc = 'orange'
