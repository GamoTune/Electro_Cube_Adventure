...
######################################################################## L'édition
    ...
    fenetre_bouton.geometry('306x306') #taille 1 bouton = taille que tu veux + 3
    fenetre.resizable(False,False)
    fenetre_bouton.attributes('-topmost',1)

    bouton_save = Button(fenetre_bouton, image=imageBoutonSolo, bg='black')
    bouton_save.place(x=0,y=0)

    bouton_retour = Button(fenetre_bouton, image=imageBoutonSolo, bg='white', command=deleteLastBlock)
    bouton_retour.place(x=103,y=0)

    bouton_niveau_up = Button(fenetre_bouton, image=imageBoutonSolo, bg='white', command=lambda:level_search("Up"))
    bouton_niveau_up.place(x=100,y=200)

    bouton_niveau_down = Button(fenetre_bouton, image=imageBoutonSolo, bg='white', command=lambda:level_search("Down"))
    bouton_niveau_down.place(x=100,y=300)
    
    bouton_niveau_left = Button(fenetre_bouton, image=imageBoutonSolo, bg='white', command=lambda:level_search("Left"))
    bouton_niveau_left.place(x=0,y=200)

    bouton_niveau_right = Button(fenetre_bouton, image=imageBoutonSolo, bg='white', command=lambda:level_search("Right"))
    bouton_niveau_right.place(x=200,y=200)
...
def save_level (event=0):
    if edit == 1:
        ...
        with open(lienfichier, "wb") as fichierNiveau:
            pickle.dump(coordsBlocs, fichierNiveau) #dicoSave
...
def loadTestZone (event=0):
    ...
    if os.path.exists(lienfichier):
        with open(lienfichier, "rb") as fichier:
            coordsBlocs = pickle.load(fichier) #dicoSave
            print(coordsBlocs)
            #coordsBlocs.pop(0)
