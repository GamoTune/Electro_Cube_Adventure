...
def menu_post_solo ():
    ...
    boutonSoloReset = Button(menuPostSolo, image=imageWIP)
            ...
            if  positionJoueur[0]+posJTestX == listeNiveau[pad]["coordsBloc"][0] and positionJoueur[1]+posJTestY == listeNiveau[pad]["coordsBloc"][1]:
                ...
                #Collision avec clé
                    ...
                    while pad2 < len(listeMonde):
                        if positionJoueur[0]+posJTestX == listeMonde[pad2]["coordsBloc"][0] and positionJoueur[1]+posJTestY == listeMonde[pad2]["coordsBloc"][1] and listeMonde[pad2]["idLevel"] == str(id_level) and listeMonde[pad2]["keyCollect"] == 0: #Permet de savoir si le bloc exite déjà dans la liste
                                touch_key(pad,pad2)
                                pad -= 1
                        pad2 += 1
...
def retour_menu (goToMenu): #Retour au menu
    global lienfichier, solo, fenetreinfosTest, fenetreeditTest, edit, ligneX, ligneY, joueur, id_level, id_level_editeur
